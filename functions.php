...
/***** Load CSS & JavaScript *****/
wp_enqueue_script('functions', get_template_directory_uri() . '/_js/functions-min.js', array('jquery'));
if (!function_exists('mh_scripts')) {
	function mh_scripts() {
		...
		wp_enqueue_style('eventurist-style', get_template_directory_uri() . '/base.css', false, '1.0');
...
if (!function_exists('mh_page_title')) {
	function mh_page_title() {
		if (!is_front_page()) {
			echo '<header class="archive-header group">' . "\n";
			...
			echo '</div>' . "\n";
			echo '<div class="archive-links">' . "\n";
			if (is_category() || is_tax()) {
				$catID = get_query_var('cat');
				$child = get_category($catID);
				$parent = $child->parent;
				$parentcat = get_category($parent);
				if($parent != 0) {
					//echo '<h3>Similar Events</h3>';
					echo '<a href = "' . get_category_link( $parent ) . '">';
					echo $parentcat->name;
					echo '</a>';
					...
					$children = get_categories( array( 'child_of' => $catID ));
					if($children) {
						//echo '<h3>Similar Events</h3>';
...
register_nav_menus( array(
	'subnav' => __( 'Sub Nav Menu', 'mh_magazine_lite_setup' ),
	'catmenu' => __( 'Category Menu', 'mh_magazine_lite_setup' ),
    'featmenu' => __( 'Featured Category Menu', 'mh_magazine_lite_setup' ),
) );
